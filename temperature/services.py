...
async def get_city_weather(
        city_name: str, client: AsyncClient
) -> tuple[datetime, int]:
...
async def parse_city_weather(city_weather: dict) -> tuple[datetime, int]:
    time_measured = datetime.strptime(
        city_weather["last_updated"], "%Y-%m-%d %H:%M"
    )
