...
async def update_city_temperature(db: AsyncSession = Depends(dependecies.get_db)):
    async with httpx.AsyncClient() as client:
        cities = await db.execute(select(City))

        for city in cities.scalars():
            time_measured, current_temperature = await services.get_city_weather(city.name, client)

            temperature = await db.execute(select(CityTemperature).where(CityTemperature.city_id == city.id))
