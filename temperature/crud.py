...
async def get_city_temperature(city_name: str, client: httpx.AsyncClient):
...
#         temperatures = await asyncio.gather(*(create_temperature(
#             db_city=db_city, db=db, client=client
#         ) for db_city in all_cities))
#     return temperatures


async def create_temperature(db: AsyncSession, db_city: models.DBCity,
                             client: httpx.AsyncClient):
    city_temperature = await get_city_temperature(
        city_name=db_city.name,
        client=client
