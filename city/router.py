...
async def add_cities(city: schemas.CityCreate, db: AsyncSession = Depends(get_db)):
    return await crud.create_city(db=db, city=city)


@router.put("/cities/{city_id}/", response_model=schemas.City)
async def add_cities(city_id: int, city: schemas.CityCreate, db: AsyncSession = Depends(get_db)):
    return await crud.update_city(db=db, city=city, city_id=city_id)


@router.delete("/cities/{city_id}/")
async def add_cities(city_id: int, db: AsyncSession = Depends(get_db)):
