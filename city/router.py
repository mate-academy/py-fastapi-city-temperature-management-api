...
@router.post("/", response_model=schemas.CityCreateUpdate, response_class=PrettyJSONResponse)
def create_city(city: schemas.CityCreateUpdate, db: Session = Depends(get_db)):
    return crud.create_city(db=db, city=city)


@router.get("/", response_model=list[schemas.CityRead], response_class=PrettyJSONResponse)
def read_cities(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    cities = crud.get_cities(db=db, skip=skip, limit=limit)
    return cities


@router.get("/{city_id}", response_model=schemas.CityRead, response_class=PrettyJSONResponse)
def read_city(city_id: int, db: Session = Depends(get_db)):
    city = crud.get_city(db=db, city_id=city_id)
    return city


@router.put("/{city_id}", response_model=schemas.CityRead, response_class=PrettyJSONResponse)
