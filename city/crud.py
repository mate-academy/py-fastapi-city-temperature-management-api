...
async def check_city(db: AsyncSession, city_id: int) -> CursorResult:
    result = await db.execute(select(models.City).where(models.City.id == city_id))  # type: ignore
    if result is None:
...
async def get_city_by_name(db: AsyncSession, city_name: str) -> Sequence[models.City]:
    result = await db.execute(select(models.City).where(models.City.name == city_name))  # type: ignore
    return result.scalar()
