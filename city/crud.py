...
def update_city(db: Session, db_city: models.City, city: schemas.CityCreate) -> models.City:
...
def delete_city(db: Session, db_city: models.City) -> str:
    db.delete(db_city)
    db.commit()
...
def get_temperature_records(db: Session, skip: int = 0, limit: int = 100) -> List[models.Temperature]:
    return db.query(models.Temperature).offset(skip).limit(limit).all()


def get_specific_city_temperature_records(db: Session, city_id: int) -> List[models.Temperature]:
